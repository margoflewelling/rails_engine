...
desc "All"
 ...
 # Need to convert Unit_price to dollars, works other than that
 # desc "Import all csv files"
 #    task :todo => :environment do
 #      files = ["Merchants", "Item", "Customer", "Invoice", "InvoiceItem,", "Transaction"]
 #      files.each do |file|
 #        filename = File.join Rails.root, "/csv/#{file}.csv"
 #        CSV.foreach(filename, headers: true) do |row|
 #          Object.const_get(file).create(row.to_h)
 #        end
 #    end 
 #  end
...
desc "Import customers from csv"
  task :customers => :environment do
    filename = File.join Rails.root, "/csv/Customer.csv"
     CSV.foreach(filename, headers: true) do |row|
...
desc "Import invoice items from csv"
 task :invoice_items => :environment do
   filename = File.join Rails.root, "/csv/InvoiceItem.csv"
   CSV.foreach(filename, headers: true) do |row|
 ...
 desc "Import invoices from csv"
  task :invoices => :environment do
    filename = File.join Rails.root, "/csv/Invoice.csv"
    CSV.foreach(filename, headers: true) do |row|
  ...
  desc "Import items from csv"
   task :items => :environment do
     filename = File.join Rails.root, "/csv/Item.csv"
     CSV.foreach(filename, headers: true) do |row|
   ...
   desc "Import merchants from csv"
    task :merchants => :environment do
      filename = File.join Rails.root, "/csv/Merchant.csv"
      CSV.foreach(filename, headers: true) do |row|
    ...
    desc "Import transactions from csv"
     task :transactions => :environment do
       filename = File.join Rails.root, "/csv/Transaction.csv"
       CSV.foreach(filename, headers: true) do |row|
